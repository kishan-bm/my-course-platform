...
// --- FIX: Removed unused FaGoogle icon ---
import { FaEye, FaEyeSlash, FaCheck, FaTimes } from 'react-icons/fa';
// --- FIX: Added correct Event Types ---
import { useState, useEffect, useRef, FormEvent, MouseEvent, KeyboardEvent } from 'react';
...
export default function RegisterForm() {
  ...
  const handleSendOtp = async (e: MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      setOtpSent(true);
      setCountdown(30);
    } catch (error) { // --- FIX: Improved error handling ---
  ...
  const handleVerifyOtp = async (e: MouseEvent<HTMLButtonElement> | KeyboardEvent<HTMLInputElement>) => {
    e.preventDefault();
    setOtpError('');
        ...
        {otpSent && !isOtpVerified && (
          <div className="space-y-2 pt-2">
            ...
            <div className="flex justify-between space-x-2">
              {otp.map((digit, index) => (
                <input key={index} ref={(el) => { if (el) otpInputs.current[index] = el; }} type="text" maxLength={1} value={digit} onChange={(e) => handleOtpChange(index, e.target.value)} onKeyDown={(e) => handleOtpKeyDown(index, e)} className={`w-1/4 p-2 text-center border rounded-md ${ otpError ? 'border-red-500' : 'border-gray-300'}`}/>
              ))}
            </div>
